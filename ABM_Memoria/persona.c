...
///para asignar id porque deberia brindar el siguiente al maximo
static int idPer=0;
static int generarId(void)
{
    return idPer++;
...
int parserPersonas(char* nombreF, Persona* personaArray[], int len)
{
    ...
    {
        index=0;
            ///fscanf(fp, "%[^\n]\n", aux); cualquiera de estas 2 es valida
            fgets(aux, 512, fp);
            do
            {
                ...
                if(r == 4)
                {
                    ///printf(" %s %s %s %s\n", auxID, auxNombre, auxApellido, auxEdad); para imprimir todos
