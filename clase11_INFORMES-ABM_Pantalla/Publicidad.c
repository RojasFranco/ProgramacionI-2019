...
int pub_ordenarDiasMenorDiez(Publicidad* pPublicidad, Pantalla* pPantalla, int len)
{
    ...
    for (i=0; i<len; i++)
    {
        if(pPublicidad[i].dias<=10)
        {
            ID=pPublicidad[i].idPublicidad;
            posicionPantalla=pan_getByID(pPantalla, len, ID);
            if(posicionPantalla>=0 && pPantalla[posicionPantalla].isEmpty==0)
            {
                printf("\n\Nombre:%s\n", pPantalla[posicionPantalla].nombre);
                printf("Precio:%f\n", pPantalla[posicionPantalla].precio);
                printf("Dias:%d \n", pPublicidad[i].dias);
                printf("Precio total: %f\n\n", pPublicidad[i].dias*pPantalla[posicionPantalla].precio);
